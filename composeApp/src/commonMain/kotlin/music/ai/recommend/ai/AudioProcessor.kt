...
class AudioProcessor {
    ...
    fun extractFeatures(audioData: FloatArray): FloatArray {
        // Buffers are now LOCAL to be thread-safe for parallel processing
        val fftInputReal = FloatArray(N_FFT)
        val fftInputImag = FloatArray(N_FFT)
        ...
        val spectrogram = computeMelSpectrogram(paddedAudio, fftInputReal, fftInputImag, powerSpectrum)
        return logMel(spectrogram)
    }

    private fun computeMelSpectrogram(
        audio: FloatArray,
        fftInputReal: FloatArray,
        fftInputImag: FloatArray,
