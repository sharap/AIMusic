...
data class MixTrack(
    ...
    override fun equals(other: Any?) = other is MixTrack && other.path == path
    override fun hashCode() = path.hashCode()
}

/** A listen, as the mix sees it: [fraction] of the track reached, at [playedAt]. */
data class MixListen(val songPath: String, val playedAt: Long, val fraction: Float)
...
object DailyMix {
    ...
    fun build(
        ...
        recentlyOffered: Set<String> = emptySet(),
        seed: Long,
        now: Long,
        size: Int = DEFAULT_SIZE
    ): List<String> {
        if (tracks.size <= size) return tracks.map { it.path }
        ...
        val available = tracks.filter { track ->
            track.path !in recentlyOffered &&
                ...
                ((weights[track.path] ?: 0f) > REJECTED || track.path in favourites)
        }.ifEmpty { tracks }
        ...
        return order(picked.map { byPath.getValue(it) })
    }
     ...
     */
    internal fun tasteWeights(listens: List<MixListen>, favourites: Set<String>, now: Long): Map<String, Float> {
        ...
        for (listen in listens) {
            ...
            weights[listen.songPath] = (weights[listen.songPath] ?: 0f) + base * decay
        }
        for (path in favourites) weights[path] = (weights[path] ?: 0f) + FAVOURITE_WEIGHT
        return weights
    }
     ...
     */
    private fun pickAnchors(tracks: List<MixTrack>, weights: Map<String, Float>, random: Random): List<MixTrack> {
        val liked = tracks.filter { (weights[it.path] ?: 0f) > 0f }
        val pool = liked.ifEmpty { tracks }
        val anchors = ArrayList<MixTrack>(ANCHORS)
        val remaining = pool.toMutableList()
        repeat(minOf(ANCHORS, pool.size)) {
            val total = remaining.sumOf { (weights[it.path] ?: 0f).toDouble().coerceAtLeast(0.01) }
            var cut = random.nextDouble() * total
            var chosen = remaining.lastIndex
            for ((i, track) in remaining.withIndex()) {
                cut -= (weights[track.path] ?: 0f).toDouble().coerceAtLeast(0.01)
                if (cut <= 0) {
                    chosen = i
                    break
                }
    ...
    private fun shortlist(
        ...
        val candidates = tracks.map { track ->
            ...
            val taste = (weights[track.path] ?: 0f).coerceIn(-1f, 1f)
            Candidate(
                track = track,
     ...
     */
    private fun select(shortlist: List<Candidate>, size: Int, now: Long, random: Random): List<String> {
        ...
        return picked.map { it.track.path }
    }
     ...
     */
    private fun order(tracks: List<MixTrack>): List<String> {
        if (tracks.size <= 2) return tracks.map { it.path }
        ...
        val ordered = ArrayList<String>(tracks.size)
        ordered += current.path
